...
def get_userlist(gid: int) -> set:
    ...
    users.update(grp.getgrgid(gid).gr_mem)
...
def main() -> int:
    ...
    if os.path.exists(config_file):
        with open(config_file, "r") as fobj:
            try:
                ...
                message(
                    syslog.LOG_CRIT,
                    "Config {0} is invalid, exiting...".format(config_file),
                )
                return os.EX_CONFIG
    else:
        message(
            syslog.LOG_CRIT, "Config {0} does not exist, exiting...".format(config_file)
        )
        return os.EX_CONFIG
    ...
    if args.list not in config["lists"]:
        message(
            syslog.LOG_CRIT,
            "Configuration for list {0} does not exist, exiting...".format(args.list),
        )
        return os.EX_NOUSER
    ...
    if gid:
        ...
        message(
            syslog.LOG_CRIT, "No GID and no users for {0}, exiting...".format(args.list)
        )
        return os.EX_CONFIG
    ...
    if list_config.get("allowed_senders") is None:
        message(
            syslog.LOG_WARNING,
            "No allowed_senders defined for {0}, allowing all mail".format(args.list),
        )
    else:
        allowed_users = []
        ...
        if from_addr not in allowed_users:
            message(
                syslog.LOG_NOTICE,
                "{0} doesn't have permission to mail to {1}".format(
                    from_addr, args.list
                ),
            )
            return os.EX_NOPERM
